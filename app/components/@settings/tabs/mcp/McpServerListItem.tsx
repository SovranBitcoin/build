...
export default function McpServerListItem({
  ...
  const parameters =
    (toolSchema.parameters as ToolParameters)?.jsonSchema.properties || {};
  const requiredParams =
    (toolSchema.parameters as ToolParameters)?.jsonSchema.required || [];

  return (
    <div className="mt-2 ml-4 p-3 rounded-md bg-bolt-elements-background-depth-2 text-xs">
      <div className="flex flex-col gap-1.5">
        <h3
          className="text-bolt-elements-textPrimary font-semibold truncate"
          title={toolName}
        >
          {toolName}
        </h3>
              ...
              {Object.entries(parameters).map(([paramName, paramDetails]) => (
                <li key={paramName} className="break-words">
                  <div className="flex items-start">
                    ...
                    <span className="mx-2 text-bolt-elements-textSecondary">
                      •
                    </span>

                    <div className="flex-1">
                      {paramDetails.type && (
                        <span className="text-bolt-elements-textSecondary italic">
                          {paramDetails.type}
                        </span>
                      )}
