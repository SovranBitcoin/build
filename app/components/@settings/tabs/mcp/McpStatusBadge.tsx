...
export default function McpStatusBadge({
  ...
  const { styles, label, icon, ariaLabel } = useMemo(() => {
    const base =
      "px-2 py-0.5 rounded-full text-xs font-medium flex items-center gap-1 transition-colors";
    ...
    const config = {
      checking: {
        ...
        icon: (
          <span
            className="i-svg-spinners:90-ring-with-bg w-3 h-3 text-current animate-spin"
            aria-hidden="true"
          />
        ),
      },
      available: {
        ...
        icon: (
          <span
            className="i-ph:check-circle w-3 h-3 text-current"
            aria-hidden="true"
          />
        ),
      },
      unavailable: {
        ...
        icon: (
          <span
            className="i-ph:warning-circle w-3 h-3 text-current"
            aria-hidden="true"
          />
        ),
      },
