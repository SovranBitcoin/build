...
export default function McpTab() {
  ...
  const checkServersAvailabilities = useMCPStore(
    (state) => state.checkServersAvailabilities,
  );
  ...
  useEffect(() => {
    if (!isInitialized) {
      initialize().catch((err) => {
        setError(
          `Failed to initialize MCP settings: ${err instanceof Error ? err.message : String(err)}`,
        );
  ...
  const checkServerAvailability = async () => {
    ...
    try {
      ...
      setError(
        `Failed to check server availability: ${e instanceof Error ? e.message : String(e)}`,
      );
    } finally {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <section aria-labelledby="server-status-heading">
        <div className="flex justify-between items-center mb-3">
          ...
          <button
            ...
            disabled={
              isCheckingServers || !parsedConfig || serverEntries.length === 0
            }
      ...
      <section aria-labelledby="config-section-heading">
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea
              ...
              className={classNames(
                ...
                "border",
                error
                  ? "border-bolt-elements-icon-error"
          ...
          <div>
            {error && (
              <p className="mt-2 mb-2 text-sm text-bolt-elements-icon-error">
                {error}
              </p>
            )}
          </div>
          ...
          <div className="mt-2 text-sm text-bolt-elements-textSecondary">
            The MCP configuration format is identical to the one used in Claude
            Desktop.
            <a
