...
export const ColorSchemeDialog: React.FC<ColorSchemeDialogProps> = ({
  ...
  const [features, setFeatures] = useState<string[]>(
    designScheme?.features || defaultDesignScheme.features,
  );
  const [font, setFont] = useState<string[]>(
    designScheme?.font || defaultDesignScheme.font,
  );
        ...
        {paletteRoles.map((role) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="font-semibold text-bolt-elements-textPrimary transition-colors">
                {role.label}
              </div>
              ...
              <button
                ...
                style={{
                  ...(f.key === "gradient" && {
                    ...
                    color: isSelected
                      ? "white"
                      : "var(--bolt-elements-textSecondary)",
                  }),
                }}
              >
                <div className="flex flex-col items-center gap-4">
                    ...
                    {f.key === "border" && (
                      <div
                        className={`w-6 h-6 rounded-lg transition-all duration-200 ${
                          isSelected
                            ? "border-3 border-current opacity-90"
                            : "border-2 border-current opacity-70"
                        }`}
                      />
                    )}
                    {f.key === "gradient" && (
                  ...
                  <div className="text-center">
                    ...
                    {isSelected && (
                      <div className="mt-2 w-8 h-1 bg-current rounded-full mx-auto opacity-60" />
                    )}
                  </div>
                </div>
              </button>
          ...
          <div className="py-4 px-4 min-w-[480px] max-w-[90vw] max-h-[85vh] flex flex-col gap-6 overflow-hidden">
            <div className="">
              ...
              <DialogDescription className="text-bolt-elements-textSecondary leading-relaxed">
                Customize your color palette, typography, and design features.
                These preferences will guide the AI in creating designs that
                match your style.
              </DialogDescription>
            </div>
            ...
            <div className="flex justify-between items-center">
              <div className="text-sm text-bolt-elements-textSecondary">
                {Object.keys(palette).length} colors • {font.length} fonts •{" "}
