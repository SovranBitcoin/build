...
import { RoutstrModelDialog } from "./RoutstrModelDialog";
...
const isModelLikelyFree = (
  ...
  if (
    ...
    model.label.includes("in:$0.00") &&
    model.label.includes("out:$0.00")
  ) {
    return true;
  }
  ...
  if (
    ...
    model.label.toLowerCase().includes("free")
  ) {
    return true;
  }
...
export const ModelSelector = ({
  ...
  const [routstrModelDialogOpen, setRoutstrModelDialogOpen] = useState(false);
    ...
    <div className="flex gap-2 flex-col sm:flex-row">
      ...
      >
        <div
          ...
          onClick={() => {
            // Special case: if provider is Routstr, open model dialog instead
            if (provider?.name === "Routstr") {
              setRoutstrModelDialogOpen(true);
            } else {
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      {isModelLikelyFree(modelOption, provider?.name) && (
                        <span
                          className="i-ph:gift text-xs text-purple-400 ml-2"
                          title="Free model"
                        />
                      )}
                    </div>
      ...
      {/* Routstr Model Dialog */}
      <RoutstrModelDialog
        isOpen={routstrModelDialogOpen}
        onClose={() => setRoutstrModelDialogOpen(false)}
        models={modelList}
        selectedModel={model}
