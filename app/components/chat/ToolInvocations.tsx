...
const jsonHighlighter: HighlighterGeneric<BundledLanguage, BundledTheme> =
  import.meta.hot?.data.jsonHighlighter ??
  ...
  return (
    <div
      className={classNames(
        "text-xs rounded-md overflow-hidden mcp-tool-invocation-code",
        className,
      )}
...
const ToolResultsList = memo(
  ({ toolInvocations, toolCallAnnotations, theme }: ToolResultsListProps) => {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ...
        <ul className="list-none space-y-4">
          {toolInvocations.map((tool, index) => {
            ...
            const isErrorResult = [
              ...
              TOOL_EXECUTION_DENIED,
              TOOL_EXECUTION_ERROR,
            ].includes(tool.toolInvocation.result);
            ...
            return (
              ...
              >
                <div className="flex items-center gap-1.5 text-xs mb-1">
                  ...
                  <div className="text-bolt-elements-textPrimary font-semibold">
                ...
                <div className="ml-6 mb-2">
                  ...
                  <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-3 rounded-md">
                    <JsonCodeBlock
                      className="mb-0"
                      code={JSON.stringify(tool.toolInvocation.args)}
                      theme={theme}
                    />
                  </div>
                  ...
                  <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-3 rounded-md">
                    <JsonCodeBlock
                      className="mb-0"
                      code={JSON.stringify(tool.toolInvocation.result)}
                      theme={theme}
                    />
                  </div>
                </div>
      ...
      >
        <ul className="list-none space-y-4">
          {toolInvocations.map((tool, index) => {
            ...
            const { toolName, toolCallId } = tool.toolInvocation;
            const annotation = toolCallAnnotations.find(
              (annotation) => annotation.toolCallId === toolCallId,
            );
                  ...
                  <div key={toolCallId} className="flex flex-col gap-1">
                    <div className="flex items-center text-bolt-elements-textSecondary font-semibold text-sm">
                      <button
                        ...
                        aria-label={
                          expanded[toolCallId] ? "Collapse" : "Expand"
                        }
                        tabIndex={0}
                        type="button"
                      >
