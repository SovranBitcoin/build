    ...
    while (i < input.length) {
      if (input.startsWith(BOLT_QUICK_ACTIONS_OPEN, i)) {
        ...
        if (actionsBlockEnd !== -1) {
          const actionsBlockContent = input.slice(
            i + BOLT_QUICK_ACTIONS_OPEN.length,
            actionsBlockEnd,
          );

          // Find all <bolt-quick-action ...>label</bolt-quick-action> inside
          const quickActionRegex =
            /<bolt-quick-action([^>]*)>([\s\S]*?)<\/bolt-quick-action>/g;
          let match;
          const buttons = [];

          while (
            (match = quickActionRegex.exec(actionsBlockContent)) !== null
          ) {
          ...
          if (closeIndex !== -1) {
            ...
            if ("type" in currentAction && currentAction.type === "file") {
              ...
              if (
                ...
                !currentAction.filePath.endsWith(".md")
              ) {
...
function createQuickActionElement(
  ...
  const elementProps = [
    ...
    ...Object.entries(props).map(
      ([key, value]) => `data-${camelToDashCase(key)}=${JSON.stringify(value)}`,
    ),
  ];

  console.log(
    "elementProps",
    `<button ${elementProps.join(" ")}>${label}</button>`,
  );
