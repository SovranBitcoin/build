...
export type StreamableHTTPServerConfig = z.infer<
  typeof streamableHTTPServerConfigSchema
>;
...
export class MCPService {
  ...
  private _validateServerConfig(
    ...
    if (hasStdioField && hasUrlField) {
      throw new Error(
        `cannot have "command" and "url" defined for the same server.`,
      );
    }
    ...
    if (hasUrlField && !config.type) {
      throw new Error(
        `missing "type" field, only "sse" and "streamable-http" are valid options.`,
      );
    }
  ...
  private async _createStreamableHTTPClient(
    ...
    logger.debug(
      `Creating Streamable-HTTP client for ${serverName} with URL: ${config.url}`,
    );
  ...
  private async _createSSEClient(
    ...
    logger.debug(
      `Creating SSE client for ${serverName} with URL: ${config.url}`,
    );
    ...
    for (const [toolName, tool] of Object.entries(tools)) {
      if (this._tools[toolName]) {
        ...
        if (existingServerName && existingServerName !== serverName) {
          logger.warn(
            `Tool conflict: "${toolName}" from "${serverName}" overrides tool from "${existingServerName}"`,
          );
        }
      }
  ...
  private async _createMCPClient(
    ...
    const validatedConfig = this._validateServerConfig(
      serverName,
      serverConfig,
    );

    if (validatedConfig.type === "stdio") {
      return await this._createStdioClient(
        serverName,
        serverConfig as STDIOServerConfig,
      );
    } else if (validatedConfig.type === "sse") {
      return await this._createSSEClient(
        serverName,
        serverConfig as SSEServerConfig,
      );
    } else {
      return await this._createStreamableHTTPClient(
        serverName,
        serverConfig as StreamableHTTPServerConfig,
      );
    }
  }
  ...
  private async _createClients() {
    ...
    const createClientPromises = Object.entries(
  ...
  async checkServersAvailabilities() {
    ...
    const checkPromises = Object.entries(this._mcpToolsPerServer).map(
      async ([serverName, server]) => {
        let client = server.client;

        try {
          logger.debug(
            `Checking MCP server "${serverName}" availability: start`,
          );

          if (!client) {
            client = await this._createMCPClient(
              serverName,
              this._config?.mcpServers[serverName],
            );
          }
  ...
  private async _closeClients(): Promise<void> {
    const closePromises = Object.entries(this._mcpToolsPerServer).map(
      async ([serverName, server]) => {
        if (!server.client) {
          return;
        }
      ...
      parts.map(async (part) => {
        ...
        if (toolInvocation.result === TOOL_EXECUTION_APPROVAL.APPROVE) {
          ...
          if (toolInstance && typeof toolInstance.execute === "function") {
            logger.debug(
              `calling tool "${toolName}" with args: ${JSON.stringify(toolInvocation.args)}`,
            );
